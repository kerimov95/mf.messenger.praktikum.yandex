    ...
    setTimeout(() => {
      this.props.chats?.forEach((chat) => {
        ...
        (ul as HTMLLIElement).addEventListener('click', async () => {
          ...
          ws.messageEvent(async (event: any) => {
    ...
    return compile(template, {
      ...
      editAvatar: new ItemButtonComponent({
        ...
        onClick: async () => {
          ...
          if (data && data.file && this.props.currentChat?.id) {
            ...
            if (users.length > 0) {
              ...
              if (usersId && usersId?.length > 0 && this.props.currentChat) {
                if (this.props.currentChat?.id) {
                  this.chatApi.addUsersToChat(usersId,
        ...
        onClick: async () => {
          ...
          if (chatId) {
            ...
            if (usersId && usersId.length > 0) {
              this.chatApi.deleteUsersFromChat(usersId, chatId)
                  .then((res) => {
                    if (res.code === 200) {
                      if (usersId?.length === 1) {
      ...
      deleteChatButton: new ItemButtonComponent({
        ...
        onClick: async () => {
          const chatId = this.props.currentChat?.id;
          if (chatId) {
            const status = await this.chatApi.deleteChat(chatId);
            if (status.code === 200) {
              alert('Чат удален');
              this.setProps({
        ...
        onClick: () => {
          if (this.currentWs?.check()) {
            ...
            if (message && message.value) {
              this.currentWs.sendMessage(message.value);
            }
          }
        },
      }).render(),
